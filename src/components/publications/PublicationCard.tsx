...
export default function PublicationCard({ publication, publisher, showFavorite, isFavorited, isAuthenticated }: Props) {
  ...
  return (
    <div className="group relative flex flex-col bg-white rounded-xl border border-gray-200 hover:border-purple-300 hover:shadow-md transition-all overflow-hidden">
      {/* Link "esticado" — invisível, cobre o cartão todo. Fica por baixo (z-0) do conteúdo
          visual, que tem pointer-events-none para deixar os cliques passarem para aqui,
          exceto no botão de favorito, que reativa pointer-events para se manter clicável
          independentemente (evita ter <button> aninhado dentro de <a>, HTML inválido). */}
      <Link
      ...
      <div className="pointer-events-none relative z-[1] flex flex-col flex-1">
        ...
        <div className="flex flex-col flex-1 p-4 gap-2">
          ...
          {/* Publisher avatar + favorito + rodapé */}
          <div className="flex items-center justify-between mt-1 pt-2 border-t border-gray-100 gap-2">
            <div className="flex items-center gap-2 min-w-0 pointer-events-auto">
              {publisher && <PublisherAvatar publisher={publisher} size={24} showName showBadge />}
              {showFavorite && (
                <FavoriteButton
