...
type Props = { userId: string; showDisclaimer?: boolean; showIdentity?: boolean; productTitle?: string };

// Normaliza para o formato esperado pelo wa.me (código do país + número, só dígitos).
// Números portugueses guardados sem indicativo (9 dígitos, começados por 9) recebem o
// prefixo 351 automaticamente.
...
export default async function ContactInfo({ userId, showDisclaimer = true, showIdentity = true, productTitle }: Props) {
    ...
    <div className="rounded-xl bg-purple-50 border border-purple-100 p-5 space-y-4">
      ...
      <div className="space-y-2">
        {typedContact.telefone && (
          <>
            <a
              href={`https://wa.me/${toWhatsAppNumber(typedContact.telefone)}?text=${encodeURIComponent(
                productTitle
                  ? `Olá, vi o seu anúncio no Enable Bank sobre ${productTitle} e gostaria de saber mais.`
                  : "Olá, vi o seu anúncio no Enable Bank e gostaria de saber mais."
              )}`}
              target="_blank"
            ...
            >
              ...
              <div>
                ...
                <p className="text-sm font-medium text-gray-900 group-hover:text-green-600">Contactar agora</p>
              </div>
            </a>
            <a
